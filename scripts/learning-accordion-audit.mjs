...
const outputDir = new URL("../artifacts/learning-accordion-audit/", import.meta.url);
...
await page.goto(`${baseUrl}/corporate-trainings`, { waitUntil: "networkidle" });

const section = page.locator("#programs");
const groups = page.locator("details.learning-group");
const summaries = page.locator("details.learning-group > summary");
const summaryCount = await summaries.count();

if (summaryCount < 2) throw new Error("Expected at least two learning groups");

const measure = () => groups.evaluateAll((nodes) => nodes.map((node) => ({
